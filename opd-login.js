...
const ROLE_PAGES    = { opd_nurse: 'opd-queue.html', opd_admin: 'opd-admin.html' };
...
function _lsKey(u){ return `opd_atx_${btoa(u.toLowerCase())}`; }
...
async function attemptLogin(){
  ...
  try{
    ...
    const authed = user.session_token
      ...
      : sb;
    authed.from('audit_log').insert({ts:new Date().toISOString(),user_name:user.username,user_role:selectedRole,action:'Login',details:`OPD module | IP:${_ip}`}).then(()=>{}).catch(()=>{});
