...
const SESSION_KEY  = 'muujiza_doctor_session';
...
try{
  ...
  if(s?.token && Date.now()<s.expiresAt && s.role==='doctor') window.location.replace('doctor-consultation.html');
}catch{}
...
function _lsKey(u){ return `doc_atx_${btoa(u.toLowerCase())}`; }
...
function recordFailure(u){ const d=getAttempts(u); d.count=Math.min((d.count||0)+1,MAX_ATTEMPTS); if(d.count>=MAX_ATTEMPTS) d.lockedUntil=Date.now()+LOCKOUT_MS; try{localStorage.setItem(_lsKey(u),JSON.stringify(d));}catch{} return d; }
...
function updateDots(u){
  ...
  const r=MAX_ATTEMPTS-d.count;
  document.getElementById('attemptLabel').textContent=r>0?`${r} attempt${r!==1?'s':''} remaining`:'Account locked';
}
...
async function attemptLogin(){
  ...
  try{
    ...
    const user=data[0];
    if(user.role !== 'doctor')
      throw new Error(`Your role is "${user.role}". This portal is for doctors only.`);
    ...
    sessionStorage.setItem(SESSION_KEY, JSON.stringify({
      ...
      role:'doctor', token:user.session_token||null,
    ...
    const authed=user.session_token
      ...
      : sb;
    authed.from('audit_log').insert({ts:new Date().toISOString(),user_name:user.username,user_role:'doctor',action:'Login',details:`Doctor portal | IP:${_ip}`}).then(()=>{}).catch(()=>{});
    window.location.replace('doctor-consultation.html?t='+Date.now());
