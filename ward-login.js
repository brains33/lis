// ============================================================
// MU'UJIZA HMS — ward-login.js
// Roles: ward_nurse -> ward-queue.html, ward_admin -> ward-admin.html
// Same auth pattern as opd-login.js / pharmacy-login.js:
// authenticate_user RPC + session_token used as x-lis-token on
// every subsequent request from the ward module.
// ============================================================
...
const ROLE_PAGES    = { ward_nurse: 'ward-queue.html', ward_admin: 'ward-admin.html' };
...
// Brute-force protection (namespaced to ward module so attempt counts
// don't collide with opd/pharmacy lockouts for a shared-username staff member)
function _lsKey(u){ return `wrd_atx_${btoa(u.toLowerCase())}`; }
...
async function attemptLogin(){
  ...
  try{
    ...
    const authed = user.session_token
      ...
      : sb;
    authed.from('audit_log').insert({ts:new Date().toISOString(),user_name:user.username,user_role:selectedRole,action:'Login',details:`Ward/IPD module | IP:${_ip}`}).then(()=>{}).catch(()=>{});
